<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    } elseif (!is_valid_state_name($state['name'])) {
      $errors[] = "State name must contains only characters, spaces and the symbol(-).";
    }
  ...
  function is_valid_phonenumber($value) {
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    if (is_blank($salesperson['phone'])) {
      ...
      $errors[] = "Phone number must be less than 255 digits.";
    } elseif(!is_valid_phonenumber($salesperson['phone'])) {
